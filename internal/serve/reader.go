...
// orderStreamReader adapts a diataxis OrderStream bidi stream as an io.Reader.
// Each Read call consumes from the current message buffer, receiving the next
// OrderStreamRequest when the buffer is exhausted.
type orderStreamReader struct {
	stream pb.OrdererService_OrderStreamServer
	buf    []byte
}

// Read fills p from the stream, receiving new messages as needed.
func (r *orderStreamReader) Read(p []byte) (int, error) {
	if len(r.buf) == 0 {
		req, err := r.stream.Recv()
		if err != nil {
			return 0, err
